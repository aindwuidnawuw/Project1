...
def recover_d_sig(d:int, r:int, s:int) -> int:

    return ((1 + d) * s + r * d) % n

def recover_d(r:int, s:int, k:int) -> int:
    denom = (s + r) % n
    if denom == 0:
        raise ZeroDivisionError("s + r == 0 mod n")
    inv = pow(denom, -1, n)
    d_rec = ((k - s) % n) * inv % n
    return d_rec
...
if __name__ == "__main__":
    print(" 两个用户用相同的 k；攻击者已知 d1 并恢复 d2")
    def gen_key():
        while True:
    ...
    forced_k = int.from_bytes(os.urandom(16), 'big')
    print("k =", hex(forced_k))

    msg1 = b"abcde"
    msg2 = b"abcdefg"

    r1, s1, k1 = sm2sign_return_k(d1, msg1, forced_k=forced_k)
    r2, s2, k2 = sm2sign_return_k(d2, msg2, forced_k=forced_k)

    print("签名1 (r1,s1) =", r1, s1)
    print("签名2 (r2,s2) =", r2, s2)
    print("k1 与 k2 在模 n 下是否相等?", (k1 - k2) % n == 0)

    k_recovered = recover_d_sig(d1, r1, s1)
    print("由 d1 恢复的 k =", hex(k_recovered))

    d2_rec = recover_d(r2, s2, k_recovered)
    print("恢复出的 d2 =", hex(d2_rec))

    P2_rec = scalar_mult(d2_rec, G)
    print("恢复出的 P2.x =", hex(P2_rec[0]))
    if P2_rec == P2:
        print("成功,恢复的 d2 与原始一致")
    else:
        print("失败,恢复的 d2 与原始不一致")
