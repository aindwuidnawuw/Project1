...
def sm2sign_k_returned(d: int, msg: bytes, forced_k: int = None) -> Tuple[int,int,int]:
...
def recover_d(r: int, s: int, k: int) -> int:
    denom = (s + r) % n
    if denom == 0:
        raise ZeroDivisionError("s + r == 0 (mod n), cannot invert")
    inv = pow(denom, -1, n)
    d_rec = ((k - s) % n) * inv % n
    return d_rec

if __name__ == "__main__":
    print("当 SM2 签名的随机数 k 泄露时恢复私钥")
    # 1) 生成密钥
    while True:
    ...
    forced_k = None
    r, s, k = sm2sign_k_returned(d, msg, forced_k=forced_k)
    print("签名 r =", r)
    print("签名 s =", s)
    print("泄露的 k =", k)
    d_rec = recover_d(r, s, k)

    print("恢复的私钥 d =", hex(d_rec))

    #验证 recovered d 与原 d 相同（或其公钥相同）
